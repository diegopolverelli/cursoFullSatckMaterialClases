...
btnSaludar.addEventListener("click", (evento)=>{
    contador++
    console.log("se hizo click...!!!", contador)
    if(btnSaludar.textContent==="Hola"){
        btnSaludar.textContent="Chau"
    }else{
        btnSaludar.textContent="Hola"
    }
})
...
btnGrabar.addEventListener("click", evento=>{
    ...
    let dato="dato 1"
    dato='dato 1'
    // console.log(dato+", otra cosa\n\notro renglon\n\t- elemento con tabulacion")
    dato=`Esta es una prueba, este es el dato: ${dato}... este es el nombre: ${nombre}

y este el apellido: ${apellido}
La fecha de hoy es ${new Date().toLocaleDateString()}
    ...
    divUsuario.textContent=`Usuario logueado ${nombre} ${apellido}`
    inputNombre.value=""
    inputApellido.value=""
...
btnLogout.addEventListener("click", e=>{
    e.preventDefault()
    divUsuario.textContent=`No hay usuarios logueados`
...
let btnCargarHeroes=document.createElement("a")
btnCargarHeroes.textContent="Cargar Heroes"
btnCargarHeroes.href="#"
document.body.append(btnCargarHeroes)

btnCargarHeroes.addEventListener("click", evento=>{
    evento.preventDefault()

    let ulHeroes
    if(document.getElementById("ulHeroes")){
        ulHeroes=document.getElementById("ulHeroes")
        ulHeroes.textContent=``
    }else{
        ulHeroes=document.createElement("ul")
        ulHeroes.id="ulHeroes"
    }

    heroesConVillanos.forEach(heroe=>{
...
let heroe=params.get("heroe")
let redirigir=params.get("redirigir")
if(heroe){
    console.log(heroe)
    let dato=heroesConVillanos.find(h=>h.name==heroe)
    if(dato){
        console.log(dato)
    }else{
...
btnGoogle.textContent="Ir a Google"

btnGoogle.addEventListener("click", e=>{
    if(redirigir){
        window.location.href=`https://www.${redirigir}.com`
    }else{
        window.location.href=`https://www.google.com`
    }
})

document.body.append(btnGoogle)

let btnReload=document.createElement("button")
btnReload.textContent="Recarga Página"
btnReload.addEventListener("click", e=>{
    window.location.reload()
})

document.body.append(btnReload)

btnReload.style.backgroundColor="black"
btnReload.style.color="white"
btnGoogle.classList.add("btn01")
// btnGoogle.classList.add("btn02")

setInterval(() => {
    btnGoogle.classList.toggle("btn02")
}, 1000);
